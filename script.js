...
function onAddItemSubmit(e) {
    ...
    //Check for edit mode after update than show value so  hide value code
    if(isEditMode) {
    ...
    for (const item of items) {
        if (item.firstChild.textContent === newItem) {
...
function checkUI() {
    ...
    const items = itemList.querySelectorAll('li'); // length increase
    console.log(items);
    if(items.length === 0) {
